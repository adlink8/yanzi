...
export function SongAiPanel({ slug, title }: SongAiPanelProps) {
  ...
  return (
    <section className="card space-y-4">
      <div>
        <h3 className="font-serif text-xl font-semibold">AI 解读助手</h3>
        <p className="mt-1 text-sm text-muted">围绕《{title}》做进一步提问。当前回答仅基于站内资料，不扮演本人。</p>
      </div>
      <form className="space-y-3" onSubmit={handleAsk}>
        <textarea className="min-h-28 w-full rounded-2xl border border-line bg-white/50 px-4 py-3 text-sm outline-none focus:border-accent" value={question} onChange={(event) => setQuestion(event.target.value)} placeholder="比如：这首歌为什么会让人反复回听？" />
