...
2. **整体解读（body）**：写这首歌独特的听感、意象、时期位置；≥160 字；不得复用“铺垫-推进-转折-收束”万能结构当正文。
3. **lyricInterpretations ≥ 3 段**（建议 4–6）：
...
1. 编排者运行 `npm run prepare:deepread-batches` 生成 `docs/deepread-batches/BATCH-*.md`
2. 每个 BATCH 分给一个子代理，**只改该文件列出的 slug**
3. 子代理完成后运行 `verify:deepread-batch`；失败则改到过线
4. 编排者汇总、再跑全量 report、提交
